    ...
    MovementEngine movement)
{
    ...
    {
        if (!double.TryParse(args[1], out double drive))
        {
            return;
        }

        if (!int.TryParse(args[2], out int attacks))
        {
            return;
        }

        if (!double.TryParse(args[3], out double weapons))
        {
            return;
        }

        if (!double.TryParse(args[4], out double shields))
        {
            return;
        }

        if (!double.TryParse(args[5], out double cargo))
        {
            return;
        }
    ...
    {
        if (!game.Planets.TryGetValue(planetName, out var planet))
        {
            return;
        }
        ...
        if (planet.Producing == ProductionType.Ship)
        {
            ...
            if (planet.ShipTypeName != prodType)
            {
                planet.ExcessProd = 0;
            }
    ...
    private static void SetRoute(Game game, Player player, string[] args)
    {
        ...
        if (!game.Planets.TryGetValue(origin, out var planet))
        {
            return;
        }
        ...
        foreach (var order in player.Orders)
        {
            switch (order.Kind)
            {
                case OrderKind.LoadCargo when order.Args.Length >= 2:
                    {
                        if (!int.TryParse(order.Args[0], out int num))
                        {
                            break;
                        }

                        var grp = player.Groups.FirstOrDefault(g => g.Number == num);
                        if (grp is null || grp.InHyperspace)
                        {
                            break;
                        }

                        var planet = game.Planets.GetValueOrDefault(grp.At);
                        if (planet is null || planet.OwnerId != player.Id)
                        {
                            break;
                        }
    ...
    private static void ApplyUpgradeOrders(Player player, Game game)
    {
        foreach (var order in player.Orders.Where(o => o.Kind == OrderKind.UpgradeGroup))
        {
            ...
            if (!player.ShipTypes.TryGetValue(grp.ShipTypeName, out var st))
            {
                continue;
            }
                ...
                case OrderKind.ReverseGroup when order.Args.Length >= 1:
                    {
                        ...
                        if (grp is not null)
                        {
                            movement.ReverseGroup(grp, game);
                        }
                ...
                case OrderKind.BreakGroup when order.Args.Length >= 2:
                    {
                        ...
                        if (!int.TryParse(order.Args[1], out int ships))
                        {
                            break;
                        }

                        var grp = player.Groups.FirstOrDefault(g => g.Number == num);
                        if (grp is null || ships <= 0 || ships >= grp.Ships)
                        {
                            break;
                        }
                ...
                case OrderKind.ScrapGroup when order.Args.Length >= 1:
                    {
                        ...
                        if (grp is null || planet is null || grp.InHyperspace)
                        {
                            break;
                        }
    ...
    private void RunCombatAndBombing(Game game, string phase)
    {
        foreach (var planet in game.Planets.Values)
        {
            ...
            if (bombing is not null)
            {
                game.Bombings.Add(bombing);
            }
        }
    }
